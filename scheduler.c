...
//allocate:allocation waiting list
int allo_wait;

//allocate:boolean deterimines if some process have recently finished
bool some_finished;
...
void insert_pro(struct process temp);
void check_allo_wait();
...
void initialize()
{
    ...
    //allocate: initialize allocation waiting list

    // allocate: initiazlize mem file
    log_mem=fopen("memory.log","w");
    fprintf(log_mem,"#At\ttime\tx\tallocated\ty\tbytes\tfor process\tz\tfrom\ti\tto\tj\n");
    fflush(log_mem);
...
void arrived()
{
    ...
    while (!allarrived)
    {
        ...
        // allocate:check if we can allocate the process

        insert_pro(temp);
    }
}

// allocate: this function traverse the allocation waiting list and allocate the oldest processes that can be allocated

void check_allo_wait()
{
    if(some_finished==false)return;
...
void insert_pro(struct process temp)
{
    ...
    pcb[temp.id]->memsize=temp.memsize;
...
//allocate: prints memory.log file
void print_mem(int id)
{
    int from=0,to=0;
    int curclk=getClk();
    fprintf(log_mem,"At\ttime\t%d\tallocated\t%d\tbytes\tfor process\t%d\tfrom\t%d\tto\t%d\n",
    curclk,pcb[id]->memsize,id,from,to);
}
